...
#include "markdown/parser/Document.h"
#include "markdown/parser/Parser.h"
#include "markdown/parser/Token.h"
...
TEST(ParseParagraphTest, OnlyTextOneSharp) {
...
TEST(ParseCodeBlockTest, OnlyTextThreeBackquote) {
  auto nodes = Parser::parse("```");
  ASSERT_EQ(nodes.size(), 1);
  auto node = nodes.at(0);
  ASSERT_EQ(node->type(), NodeType::code_block);
  auto codeBlock = (CodeBlock*)node;
  ASSERT_EQ(codeBlock->code()->str().empty(), true);
}
